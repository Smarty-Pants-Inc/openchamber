...
describe('shouldShowPwaInstallToast', () => {
    test('returns true when nothing blocks the toast', () => {
        expect(shouldShowPwaInstallToast({ seen: null, hasActiveToast: false })).toBe(true);
    });

    test('returns false once the toast was shown, dismissed, or the app installed', () => {
        expect(shouldShowPwaInstallToast({ seen: 'true', hasActiveToast: false })).toBe(false);
    });

    test('returns false when the effect already owns an active toast', () => {
        expect(shouldShowPwaInstallToast({ seen: null, hasActiveToast: true })).toBe(false);
    });

    test('treats non-"true" storage values as unset', () => {
        expect(shouldShowPwaInstallToast({ seen: 'false', hasActiveToast: false })).toBe(true);
    });
});
