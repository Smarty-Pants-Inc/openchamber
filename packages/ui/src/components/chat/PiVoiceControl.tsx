...
// Loaded on first use: the call module brings the runtime transport, not needed to render the chip.
...
export function PiVoiceControl({ sessionId, directory }: { sessionId: string; directory: string }) {
  ...
  const toggle = async () => {
    ...
    try {
      // Media is created inside the click so autoplay policy allows the remote voice.
      const media = browserPiVoiceMedia();
      const voice = await loadCall().catch((error: Error) => { media.close(); throw error; });
      const started = await voice.startPiVoiceCall(voice.piVoiceTransport(sessionId, directory), media, next => {
      ...
      if (generation.current !== owner) started.hangup(); else call.current = started;
    } catch (error) {
      if (generation.current !== owner) return;
