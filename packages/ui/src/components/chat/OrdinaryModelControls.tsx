...
export function OrdinaryModelControls({ state, className, voice }: {
  state: OrdinaryModelState; className?: string; voice?: { sessionId: string; directory: string };
}) {
  const { t } = useI18n();
  return (
    <div className={cn('flex min-w-0 items-center gap-2 typography-meta text-muted-foreground', className)} aria-live="polite">
      ...
      {voice ? <PiVoiceControl sessionId={voice.sessionId} directory={voice.directory} /> : null}
    </div>
  );
}
