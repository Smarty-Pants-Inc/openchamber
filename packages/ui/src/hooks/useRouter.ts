...
export function useRouter(): void {
  ...
  React.useEffect(() => {
    ...
    // A cleared restore pointer may drop its pending ?session= only while the page does not show that session (#113).
    setShownSessionProbe(() => useSessionUIStore.getState().currentSessionId);
