...
The global event pipeline treats `project.list.changed` as an invalidation. It coalesces refreshes, refetches `sdk.project.list()`, and publishes the successful result to the global runtime snapshot. It grants that snapshot project-membership authority only when the existing `/health.runtime` value is `smarty-oc`, the explicit Smarty-OC gateway signal. Any other or unavailable runtime leaves settings-owned project membership intact. Bootstrap, stream reconnect, and transport switches use the same runtime-key-guarded catalog path.
