...
2. If an action targets a session by ID, resolve the **session's own directory**. Consult the existing canonical `getDirectoryForSession()` resolver and global session metadata before generic child-store presence. A child may contain another directory's session, status or blocking requests; containment is not ownership. Keep child presence only as a fallback when no ownership record is indexed. Do not assume the current directory is correct.
...
An admitted managed-project catalog overrides the global New session Chat default described below. A plain draft uses its remembered admitted project or the catalog's active/first project, with that project's directory. An empty catalog leaves the project target empty. It never substitutes the old SDK directory, a removed project, or Chat through stale-directory recovery. Explicit project/worktree requests retain their existing validation. Stock runtimes keep the Chat default and recovery behavior below.
