...
Pi creation uses one opaque correlation for its single `POST /session` request and polls the matching pending-create record with the same canonical directory and correlation while that request is unresolved. The UI keeps a synthetic local pending ID only until the gateway supplies its pending-create ID, filters nonblocking extension notifications, and maps confirmation and value requests into the existing permission/question cards. Every reply carries that same directory and correlation; terminal scope/mismatch failures remove only that dialog. Completion, failure, and runtime switching abort the poll and clear only that correlation's cards, so concurrent creates cannot exchange dialogs or replies.
