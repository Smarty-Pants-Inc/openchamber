...
Pi creation uses one opaque correlation for its single `POST /session` request and polls the matching pending-create record with the same canonical directory and correlation while that request is unresolved. The UI keeps a synthetic local pending ID only until the gateway supplies its pending-create ID, filters nonblocking extension notifications, and maps confirmation and value requests into the existing permission/question cards. Every reply carries that same directory and correlation; terminal scope/mismatch failures remove only that dialog. Completion and failure stop the poll, await any in-flight observer decode and dialog replies, and clear only that correlation's cards. Runtime switching clears the visible cards but keeps a cancellation-only poll on the captured old transport, rejects both known and newly discovered dialogs there, and never aborts the already-dispatched create POST. The create owner awaits the poll and every cancellation before releasing that transport, then compensates the exact returned session and directory if creation succeeded after the switch. Pi's own dialog deadline remains the final no-responder fallback, so concurrent creates cannot exchange dialogs or replies.
