...
Pi creation uses one opaque correlation for its single `POST /session` request and polls the matching pending-create record with the same canonical directory and correlation while that request is unresolved. The UI keeps a synthetic local pending ID only until the gateway supplies its pending-create ID, filters nonblocking extension notifications, and maps confirmation and value requests into the existing permission/question cards. Every reply carries that same directory and correlation; terminal scope/mismatch failures remove only that dialog. Completion and failure abort the poll and clear only that correlation's cards. Runtime switching first rejects every currently known dialog through the captured old transport, then aborts the poll, but never aborts the already-dispatched create POST; it awaits that response and compensates the exact returned session and directory. Pi's own dialog deadline remains the final no-responder fallback, so concurrent creates cannot exchange dialogs or replies.
