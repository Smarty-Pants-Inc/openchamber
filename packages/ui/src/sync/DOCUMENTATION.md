...
The global event pipeline treats `project.list.changed` as an invalidation. It coalesces refreshes, refetches `sdk.project.list()`, and publishes the successful result to the global runtime snapshot. A valid `/health.runtime` value of `smarty-oc` grants that snapshot project-membership authority; a successful explicit non-Smarty identity releases it back to settings ownership, while failed, missing, or malformed health preserves the existing ownership. Live-only projects keep a live-only active selection in the runtime-namespaced local active cache, and live catalog bootstrap restores it when that member returns. They are materialized into settings only immediately before icon discovery or upload routes need a settings lookup; those routes revalidate the captured runtime context before dispatch. Provider lifecycle generations reject stale catalog results after unmount or a same-runtime remount. Bootstrap, stream reconnect, and transport switches use the same runtime-key-guarded catalog path.
...
9. A multi-request session operation that may need stale-runtime compensation binds once with `bindSessionOperation()`. Its remote `request`, `create`, `get`, `getMessages`, `patchMetadata`, `fork`, `delete`, and `updateTitle` calls retain the original SDK base URL, auth, and relay transport until `release()` in `finally`. Bound creation stamps the exact requested backend classification; Pi creation also keeps startup-dialog polling, session loading, and exact-directory compensation on that transport. These methods never write stores. Call `publish()` or `finalizeDeletion()` only after the final `assertCurrent()`; those local updates are synchronous, so a fork cannot leave route/global/live state partially published. `patchSessionMetadata()` likewise rechecks authority after JSON decoding and immediately before its local commit.
10. Every normal prompt, slash command, and shell dispatch awaits `session-send-preflight.ts` after optimistic insertion (where applicable) and immediately before the backend call. The web server or VS Code extension host serializes that check by directory/session, scans complete history, and may CAS-backfill only a proven legacy Pi/OMP/Codex marker. Native-to-managed, managed-to-native, changes between managed backends, mixed-history, and incomplete-history sends fail closed. A rejected optimistic prompt/command rolls back; a rejected shell command is never dispatched.
11. Codex child sessions with `metadata.ompSubagent === true` are read-only projections owned by the root app-server. Row, bulk, mobile, archive, restore, delete, rename, and retention cleanup flows exclude them. Root Codex sessions remain manageable.
...
### Pi startup-create dialogs

Pi creation uses one opaque correlation for its single `POST /session` request and polls the matching pending-create record with the same canonical directory and correlation while that request is unresolved. The UI keeps a synthetic local pending ID only until the gateway supplies its pending-create ID, filters nonblocking extension notifications, and maps confirmation and value requests into the existing permission/question cards. The presenter shows one deterministically selected prompt at a time while the others remain queued. Replies retain the raw dialog ID and correlation, while keyboard ownership uses a correlation-scoped identity. Terminal scope/mismatch failures remove only that dialog. A valid authoritative snapshot that omits a previously observed correlation ends and clears only that lane. Completion and failure stop the poll, await any in-flight observer decode and dialog cancellations, and clear only that correlation's cards. Runtime switching clears the visible cards but keeps a cancellation-only poll on the captured old transport, rejects both known and newly discovered dialogs there, and never aborts the already-dispatched create POST. A successful create makes its final bound authority check after those drains and compensates the exact returned session and directory if authority changed. Pi's own dialog deadline remains the final no-responder fallback, so concurrent creates cannot exchange dialogs or replies.
