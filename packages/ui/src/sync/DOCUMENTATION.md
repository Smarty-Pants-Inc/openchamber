...
10. Every normal prompt, slash command, and shell dispatch awaits `session-send-preflight.ts` after optimistic insertion (where applicable) and immediately before the backend call. The web server or VS Code extension host serializes that check by directory/session, scans complete history, and may CAS-backfill only a proven legacy Pi/OMP marker. Native-to-managed, managed-to-native, Pi-to-OMP, OMP-to-Pi, mixed-history, and incomplete-history sends fail closed. A rejected optimistic prompt/command rolls back; a rejected shell command is never dispatched.
