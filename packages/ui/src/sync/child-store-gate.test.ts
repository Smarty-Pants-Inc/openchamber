...
test('the bootstrap gate holds the queue, then drops denied directories and runs admitted ones', async () => {
  ...
  expect(manager.getBootstrapState('/home/user')).toBeUndefined();
  cleanup();
  manager.disposeAll();
});
