  ...
  const redeemPairingConnection = React.useCallback(async (payload: PairingConnectionPayload) => {
    ...
    try {
      ...
      const redeemBody = JSON.stringify({
        ...
        clientLabel: `${PRODUCT_NAME} Mobile`,
        clientKind: 'mobile',
        deviceName: `${PRODUCT_NAME} Mobile`,
  ...
  const submitPassword = React.useCallback(async (password: string) => {
    ...
    try {
      ...
      const loginInit = {
        ...
        body: JSON.stringify({ password, trustDevice: true, issueClientToken: true, clientLabel: `${PRODUCT_NAME} Mobile`, clientKind: 'mobile', devicePlatform: mobileDevicePlatform(), dedupeKey: mobileClientDedupeKey() }),
      };
