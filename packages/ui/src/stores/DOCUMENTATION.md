...
resolved home if target normalization keeps them absolute. Windows drive roots
remain blocked without home because downstream project normalization strips their
root separator; full drive-root support needs that shared contract corrected first.
Relative and `~` input still require home discovery. Existing project membership
and filesystem errors still block Add where applicable.
...
Foreground dispatchers mark sends with `markSending` and use `getSendableQueue()`. In-flight entries remain visible but cannot be sent twice or removed by bulk clear. An unsettled foreground attempt becomes unknown recovery work. Version-4 browser persistence retains foreground payloads for review on reload, never as ready-to-send work. `sendingIds` only tracks the live process reservation. Desktop queues use the configured host ID as runtime identity because SSH reconnects can change the forwarding URL without changing the host.
...
the migration was saving, even when the navigation save already succeeded.
...
Chat composer drafts, confirmed mentions, inline-comment drafts, and pinned sessions use the same runtime/directory/session ownership rule. Inline-comment targets can carry an explicit captured runtime for delayed native-send completion; ordinary callers retain the active-runtime default. Accepted native materialization transfers newer composer text and mentions without depending on durable-draft preference, and consumes only the original submitted context. Chat drafts use a bounded shared envelope and notify mounted composers when authoritative deletion clears their identity, preventing unmount autosave from resurrecting deleted text. New-session identities also carry their live draft ID. The persistence owner retains the latest claimed generation per runtime/path draft slot for the page lifetime, without serializing it or changing existing keys. Accepted native input consumption first lets the matching mounted editor settle newer text, then checks its saved snapshot. Older generations cannot clear or write over replacement drafts, including after navigation or epoch remount. Inline drafts enforce per-session, global-session, and serialized-byte bounds. Pins retain every valid composite key across runtimes without silent age/count eviction and are never pruned from the first startup list. Confirmed local deletion and routed deletion events clear immediately; after an authoritative baseline exists, a later complete omission also cleans persisted state. Ambiguous session-only legacy drafts and pins are not claimed.
...
the directory explorer (browse, folder creation, clone, add) does not render.
An explicit Add request while discovery is `unknown` or `unavailable` retries
discovery (`resolveProjectAddAllowed` in `lib/managed-project-add.ts`) and
opens the dialog only on a `stock` answer from the same runtime; otherwise it
shows a brief try-again toast. Web, desktop and mobile reach
...
From the first managed marker (`admitManagedCatalog`, before the session
listing is awaited) `useDirectoryStore.managedDirectories` is `[]`: every
directory is denied and the pre-discovery selection is dropped until rows are
published. Later markers and failed refreshes keep the rows already admitted.
Once published, `managedDirectories` holds the live rows.
...
saved selection per page load names the
project shown after the current turn, so session restoration is included.
A selection is keyed by its saved path (and that path's derived id) whenever
the path is known, so a cached `lastDirectory` and a later settings
