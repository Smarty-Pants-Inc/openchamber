...
Project and UI settings use successful settings synchronization as authority. Omitted fields in a complete snapshot reset to canonical client defaults, including an omitted project list becoming empty; transport or settings-load failure dispatches no synchronization event and preserves current state. Settings save responses are partial patches and must not clear unrelated in-memory preferences or local mirrors. Only the existing `/health.runtime` value `smarty-oc` may activate runtime project membership; a successful explicit non-Smarty identity releases it, while failed, missing, or malformed health leaves current ownership unchanged.

The Smarty-OC gateway has that explicit runtime identity. While its live catalog is active, canonical runtime worktree paths add and remove project members; settings supply presentation metadata such as labels, icons, colors, model preferences, and manual order. A bootstrap may adopt an incoming active ID only when it names a live member. Selecting a live-only project keeps its active ID only in the runtime-namespaced local active cache; it never sends that ID in shared settings unless the project has first been materialized into presentation settings. Live-only projects become settings entries only immediately before their icon discovery or upload route needs that lookup, so runtime membership remains authoritative. Ordinary OpenCode runtimes retain settings-owned membership and user add/remove mutations. A runtime endpoint switch clears live authority before the next catalog loads, and catalog refreshes preserve presentation metadata across additions, removals, and reconnects.
