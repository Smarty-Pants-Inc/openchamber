...
Before Smarty Code can do anything, it needs at least one AI provider connected. This page covers connecting a provider, choosing a model, and tailoring agents.
...
3. Sign in one of two ways, depending on the provider:
   ...
   - **Sign-in (device flow)** — Smarty Code shows a link and a short code. Open the link, enter the code, and approve. Smarty Code finishes connecting on its own.
...
4. Save — Smarty Code writes the provider block to OpenCode config and stores the key in OpenCode auth (literal keys) or records an `{env:VAR}` reference.
...
Provider sign-ins are stored by OpenCode, not Smarty Code, so they're shared with the OpenCode CLI. If you set the same provider in more than one place, the most specific setting wins: a per-project setting overrides your personal one.
