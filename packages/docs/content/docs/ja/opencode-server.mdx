---
title: OpenCode サーバー
description: smarty-code をローカルまたはリモートの OpenCode サーバーに接続します。
---

# OpenCode サーバー

smarty-code は OpenCode サーバーの上で動作します。デフォルトでは smarty-code がサーバーを起動するため、何もする必要はありません。このページが必要なのは、すでに実行しているサーバーに smarty-code を向けたい場合、または smarty-code が起動するサーバーを管理したい場合だけです。

## smarty-code がサーバーを見つける順序

smarty-code の起動時、次の順序でサーバーを探します。
...
- `OPENCODE_SKIP_START=true` — smarty-code に自分のサーバーを起動しないよう伝えます。

ポートだけを変えたい場合は、`OPENCODE_HOST` ではなく `OPENCODE_PORT` を設定します。

`OPENCODE_HOST` にポートがない、またはパスが含まれている場合、smarty-code はそれを無視し、自分のサーバー起動にフォールバックします。期待した接続が行われなかった場合は、起動ログの `[config]` 警告を確認してください。
...
## ログイン時に smarty-code を起動する

`startup enable` を使うと、ネイティブのユーザーサービスをインストールできます。smarty-code は macOS では `launchd`、Linux では `systemd --user`、Windows では Task Scheduler を使います。
...
別の smarty-code アプリ向けに接続リンクを作成するには、次を使います。
...
サーバーの起動中または再起動中、smarty-code は "OpenCode is restarting" 状態を表示し、準備が整うまでリクエストを一時停止します。これは起動直後や再起動直後には正常です。いつまでも消えない場合は、[OpenCode 接続](/troubleshooting/opencode-connection/) を参照してください。
