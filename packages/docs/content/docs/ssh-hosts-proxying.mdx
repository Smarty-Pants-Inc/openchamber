...
Smarty Code can read hosts from your local SSH config, the same place used by commands like `ssh work-server`.
...
Smarty Code creates a remote instance using that SSH command. When it reaches **ready**, the remote Smarty Code UI opens in the desktop app.
...
Smarty Code supports three forward types:
...
Choose **Dynamic (-D)** when you want other tools on your computer to browse through the remote machine. Smarty Code opens a local SOCKS proxy port for that SSH connection.
...
- [Remote Instances](/remote-instances/) — connect the desktop app to Smarty Code on another machine
