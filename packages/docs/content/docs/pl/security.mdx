...
smarty-code daje dostęp do Twojej maszyny i Twojego kodu, więc zabezpiecz go, zanim ktokolwiek poza Tobą będzie mógł do niego dotrzeć. Ta strona omawia hasło UI, klucze dostępu (passkeys) oraz to, co warto wiedzieć przed udostępnieniem smarty-code w sieci.

## Ustaw hasło UI

Uruchom smarty-code z hasłem, a interfejs webowy poprosi o nie:

```bash
openchamber --ui-password be-creative-here
```

Możesz też ustawić je za pomocą zmiennej środowiskowej `OPENCHAMBER_UI_PASSWORD`, zamiast podawać je w wierszu poleceń. Po zalogowaniu smarty-code zapamiętuje urządzenie na pewien czas, więc nie pyta za każdym razem.
...
Po ustawieniu hasła możesz dodać klucze dostępu (Face ID, Touch ID, klucz bezpieczeństwa) dla szybszego logowania. Dodaj je w **Settings → smarty-code → Passkeys**.
...
- Domyślnie smarty-code nasłuchuje tylko na Twojej własnej maszynie (`127.0.0.1`). Nasłuchiwanie szerzej wymaga celowej zmiany i najpierw powinieneś ustawić hasło.
...
- Jeśli umieszczasz smarty-code za własnym serwerem HTTPS, zobacz [Reverse Proxy](/pl/reverse-proxy/).
...
- [Reverse Proxy](/pl/reverse-proxy/) — uruchom smarty-code za własnym serwerem
