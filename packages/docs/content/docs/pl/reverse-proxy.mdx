---
title: Reverse proxy
description: Skonfiguruj Smarty Code poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz Smarty Code za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że Smarty Code działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli Smarty Code już kompresuje odpowiedzi.
...
- Smarty Code dostępny bezpośrednio w LAN
...
- Smarty Code kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez Smarty Code, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
