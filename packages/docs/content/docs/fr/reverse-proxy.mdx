---
title: Reverse proxy
description: Configurez correctement Smarty Code derrière Nginx, Nginx Proxy Manager ou un autre reverse proxy.
---

# Reverse proxy

Utilisez cette page si vous exécutez Smarty Code derrière Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou un autre reverse proxy.

## Avant de le proxifier

1. Confirmez d’abord que Smarty Code fonctionne en accès direct.
...
- Désactivez gzip sur le proxy si Smarty Code compresse déjà les réponses.
...
- Smarty Code joignable directement sur le LAN
...
- Smarty Code compresse les réponses HTTP avec gzip (seuil de 1 Ko).
...
Les routes de streaming SSE sont exclues de la compression par Smarty Code, mais le CDN peut encore les bufferiser. Consultez la documentation de votre CDN pour désactiver le buffering sur les chemins SSE.
