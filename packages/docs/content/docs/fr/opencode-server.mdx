...
description: Connectez smarty-code à un serveur OpenCode local ou distant.
---

# Serveur OpenCode

smarty-code fonctionne par-dessus un serveur OpenCode. Par défaut, il en démarre un pour vous, donc vous n’avez rien à faire. Vous n’avez besoin de cette page que si vous voulez pointer smarty-code vers un serveur que vous exécutez déjà, ou gérer celui qu’il démarre.

## Comment smarty-code trouve un serveur

Au démarrage, smarty-code cherche un serveur dans cet ordre :
...
Définissez ces variables avant de lancer smarty-code :
...
- `OPENCODE_SKIP_START=true` — indique à smarty-code de ne pas démarrer son propre serveur.

Si vous voulez seulement changer le port, définissez `OPENCODE_PORT` au lieu de `OPENCODE_HOST`.

Si `OPENCODE_HOST` n’a pas de port ou contient un chemin, smarty-code l’ignore et revient au démarrage de son propre serveur. Surveillez les logs de démarrage pour un avertissement `[config]` si une connexion attendue ne se fait pas.
...
## Lancer smarty-code à la connexion

Utilisez `startup enable` pour installer un service utilisateur natif. smarty-code utilise `launchd` sur macOS, `systemd --user` sur Linux et le Planificateur de tâches sur Windows.
...
Pour créer un lien de connexion pour une autre application smarty-code, utilisez :
...
Pendant que le serveur démarre ou redémarre, smarty-code affiche l’état « OpenCode redémarre » et met les requêtes en pause jusqu’à ce qu’il soit prêt. C’est normal juste après un lancement ou un redémarrage. Si l’état ne disparaît jamais, consultez [Connexion à OpenCode](/troubleshooting/opencode-connection/).
