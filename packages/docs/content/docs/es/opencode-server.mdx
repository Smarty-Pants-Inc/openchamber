...
description: Conecta smarty-code a un servidor de OpenCode local o remoto.
---

# OpenCode Server

smarty-code se ejecuta sobre un servidor de OpenCode. De forma predeterminada inicia uno por ti, así que no tienes que hacer nada. Solo necesitas esta página si quieres apuntar smarty-code a un servidor que ya ejecutas, o gestionar el que él inicia.

## Cómo encuentra smarty-code un servidor

Cuando smarty-code arranca, busca un servidor en este orden:
...
Define esto antes de iniciar smarty-code:
...
- `OPENCODE_SKIP_START=true` — le indica a smarty-code que no inicie su propio servidor.

Si solo necesitas cambiar el puerto, define `OPENCODE_PORT` en lugar de `OPENCODE_HOST`.

Si a `OPENCODE_HOST` le falta el puerto o tiene una ruta, smarty-code lo ignora y recurre a iniciar su propio servidor. Observa los registros de arranque por si aparece una advertencia `[config]` cuando una conexión que esperabas no ocurrió.
...
## Iniciar smarty-code al iniciar sesión

Usa `startup enable` para instalar un servicio nativo de usuario. smarty-code usa `launchd` en macOS, `systemd --user` en Linux y Task Scheduler en Windows.
...
Para crear un enlace de conexión para otra app de smarty-code, usa:
...
Mientras el servidor se inicia o reinicia, smarty-code muestra un estado "OpenCode is restarting" y pausa las solicitudes hasta que esté listo. Esto es normal justo después de iniciar o reiniciar. Si nunca desaparece, consulta [Conexión de OpenCode](/es/troubleshooting/opencode-connection/).
