...
Connect your GitHub account and Smarty Code can pull in issues and pull requests, start a session straight from one, and open or update PRs for you.

## Connect GitHub

1. Open **Settings → Integrations** and expand the **GitHub** card.
2. Choose **Connect GitHub**. Smarty Code shows a link and a short code.
3. Open the link, enter the code, and approve.

When it's connected, your account shows on the card. You can connect more than one account and switch between them, or disconnect at any time. If the `gh` CLI is signed in on the same machine, Smarty Code can use it as a fallback.
...
- pick an **issue** and Smarty Code names the branch after it and opens the session with the issue and its comments as the first message
...
From the **PR** tab in the [git view](/git/), you can create a pull request, update it, mark a draft as ready, or merge it. Smarty Code can generate the PR title and description from your changes.
