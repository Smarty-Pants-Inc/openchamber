---
title: Tunnel
description: Stelle smarty-code sicher für Remote- und Mobilzugriff bereit.
---

# Tunnel

Ein Tunnel ist ein öffentlicher Link zu deinem smarty-code, damit du ihn von einem normalen Browser in einem anderen Netzwerk aus erreichen kannst. Nutze `openchamber tunnel`, um einen für eine laufende Instanz zu erstellen.
...
smarty-code startet das CLI des Tunnel-Anbieters auf deinem Rechner. Installiere zuerst den Anbieter, den du verwenden möchtest:
...
Sobald der Tunnel aktiv ist, zeigt `status` eine öffentliche URL. Öffne sie oder scanne den QR-Code, um smarty-code von überall aus zu erreichen.

Standardmäßig druckt smarty-code in interaktiven TTY-Sitzungen einen QR-Code. Mit `--qr` erzwingst du die QR-Ausgabe, mit `--no-qr` deaktivierst du sie.
...
- ein aktiver Tunnel pro smarty-code-Instanz (Port)
...
- [PWA & Mobile](/mobile/) — smarty-code vom Telefon aus erreichen
