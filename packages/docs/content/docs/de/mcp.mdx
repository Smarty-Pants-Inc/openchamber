...
2. Füge einen Server hinzu und wähle seinen Typ:
   - **lokal** — smarty-code führt einen Befehl auf deinem Rechner aus. Du gibst den auszuführenden Befehl an und, falls nötig, Umgebungsvariablen.
   - **entfernt** — smarty-code verbindet sich mit einer URL, die jemand anderes hostet. Du gibst die URL und alle nötigen Header an (zum Beispiel ein Auth-Token).
