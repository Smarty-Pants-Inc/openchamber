---
title: Tüneller
description: Uzak ve mobil erişim için Smarty Code'ı güvenli biçimde açığa çıkarın.
---

# Tüneller

Bir tünel, Smarty Code'ınıza giden genel bir bağlantıdır. Böylece başka bir ağdaki normal bir tarayıcıdan erişebilirsiniz. Çalışan bir örnek için bir tane oluşturmak üzere `openchamber tunnel` kullanın.
...
Smarty Code, tünel sağlayıcısının CLI'sını sizin makinenizde başlatır. Önce kullanmak istediğiniz sağlayıcıyı kurun:
...
Tünel ayaktayken `status` bir genel URL gösterir. Onu açın ya da QR kodunu tarayın, Smarty Code'a her yerden ulaşın.

Varsayılan olarak Smarty Code, etkileşimli TTY oturumlarında bir QR kodu basar. QR çıktısını zorlamak için `--qr`, kapatmak için `--no-qr` kullanın.
...
- Smarty Code örneği başına bir aktif tünel, yani port
...
- [PWA & Mobile](/mobile/) — telefonunuzdan Smarty Code'a ulaşın
