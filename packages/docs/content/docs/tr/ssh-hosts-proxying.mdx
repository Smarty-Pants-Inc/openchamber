...
Smarty Code, yerel SSH yapılandırmanızdaki host'ları okuyabilir. Bu, `ssh work-server` gibi komutların kullandığı yerdir.
...
Smarty Code o SSH komutunu kullanarak bir uzak örnek oluşturur. **ready** noktasına geldiğinde uzak Smarty Code arayüzü masaüstü uygulamasında açılır.
...
Smarty Code üç yönlendirme tipini destekler:
...
Bilgisayarınızdaki diğer araçların uzak makine üzerinden gezmesini istiyorsanız **Dynamic (-D)** seçin. Smarty Code, o SSH bağlantısı için yerel bir SOCKS proxy portu açar.
...
- [Uzak örnekler](/remote-instances/) — masaüstü uygulamasını başka bir makinedeki Smarty Code'a bağlayın
