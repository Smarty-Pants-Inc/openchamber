---
title: Reverse Proxy
description: Smarty Code'ı Nginx, Nginx Proxy Manager veya başka bir reverse proxy arkasında doğru şekilde ayarlayın.
---

# Reverse Proxy

Smarty Code'ı Nginx, Nginx Proxy Manager, Caddy, Cloudflare ya da başka bir reverse proxy arkasında çalıştırıyorsanız bu sayfayı kullanın.

## Proxy'lemeden önce

1. Önce Smarty Code'ın doğrudan çalıştığını doğrulayın.
...
- Smarty Code yanıtları zaten sıkıştırıyorsa proxy'de gzip'i kapatın.
...
- Smarty Code LAN üzerinden doğrudan erişilebilir
...
- Smarty Code HTTP yanıtlarını gzip ile sıkıştırır, eşik 1 KB'dir.
...
SSE akış rotaları Smarty Code tarafından sıkıştırmanın dışında tutulur, ancak CDN bunları yine de buffer'layabilir. SSE yollarında buffer'lamayı nasıl kapatacağınızı CDN belgelerinden kontrol edin.
