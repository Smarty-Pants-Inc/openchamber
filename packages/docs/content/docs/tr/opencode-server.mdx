...
description: Smarty Code'ı yerel veya uzak bir OpenCode sunucusuna bağlayın.
---

# OpenCode Sunucusu

Smarty Code, bir OpenCode sunucusunun üzerinde çalışır. Varsayılan olarak sizin için bir sunucu başlatır; bu nedenle bir şey yapmanız gerekmez. Bu sayfaya yalnızca Smarty Code'ı zaten çalıştırdığınız bir sunucuya yönlendirmek veya onun başlattığı sunucuyu yönetmek istiyorsanız ihtiyacınız olur.

## Smarty Code sunucuyu nasıl bulur

Smarty Code başladığında sunucuyu şu sırayla arar:
...
Smarty Code'ı başlatmadan önce şunları ayarlayın:
...
- `OPENCODE_SKIP_START=true`, Smarty Code'a kendi sunucusunu başlatmamasını söyler.

Yalnızca bağlantı noktasını değiştirmek istiyorsanız `OPENCODE_PORT` öğesini `OPENCODE_HOST` yerine ayarlayın.

`OPENCODE_HOST` bağlantı noktasını içermiyorsa veya bir yol içeriyorsa Smarty Code bunu yok sayar ve kendi sunucusunu başlatmaya döner. Beklediğiniz bağlantı kurulmadıysa başlangıç günlüklerindeki `[config]` uyarısına bakın.
...
## Smarty Code'ı oturum açınca başlatın

Yerel bir kullanıcı hizmeti kurmak için `startup enable` kullanın. Smarty Code macOS'ta `launchd`, Linux'ta `systemd --user`, Windows'ta Task Scheduler kullanır.
...
Başka bir Smarty Code uygulaması için bağlantı bağlantısı oluşturmak üzere şunu kullanın:
...
Sunucu başlarken veya yeniden başlarken Smarty Code "OpenCode yeniden başlatılıyor" durumunu gösterir ve hazır olana kadar istekleri duraklatır. Bu, uygulama açıldıktan veya yeniden başlatıldıktan hemen sonra normaldir. Durum hiç kaybolmazsa [OpenCode bağlantısı](/troubleshooting/opencode-connection/) sayfasına bakın.
