---
title: Tunnels
description: Expose smarty-code safely for remote and mobile access.
---

# Tunnels

A tunnel is a public link to your smarty-code, so you can reach it from an ordinary browser on another network. Use `openchamber tunnel` to create one for a running instance.
...
smarty-code starts the tunnel provider CLI on your machine. Install the provider you want to use first:
...
When the tunnel is up, `status` shows a public URL. Open it, or scan the QR code, to reach smarty-code from anywhere.

By default, smarty-code prints a QR code in interactive TTY sessions. Use `--qr` to force QR output, or `--no-qr` to disable it.
...
- one active tunnel per smarty-code instance (port)
...
- [PWA & Mobile](/mobile/) — reach smarty-code from your phone
