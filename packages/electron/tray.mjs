...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return `${PRODUCT_NAME} — no active sessions`;
  const bits = [];
  ...
  return `${PRODUCT_NAME} — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    if (isLinux || process.platform === 'win32') {
      ...
      template.push(
        { label: `Show ${PRODUCT_NAME}`, click: () => onAction({ type: 'show-main-window' }) },
        { type: 'separator' },
        { label: `Quit ${PRODUCT_NAME}`, click: () => onAction({ type: 'quit' }) },
      );
    }
