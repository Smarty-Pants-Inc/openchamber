...
- `POST /api/fs/mkdir`, `write`, `rename` and `delete` resolve every existing ancestor with `realpath` (`containedPath`) and require the canonical result to stay inside the canonical base the path was admitted under; otherwise they answer `403` before touching the filesystem. `write` and `mkdir` follow the final component; `rename` and `delete` act on the final entry itself (a link inside the workspace can be removed or renamed, its target is never reached). A dangling link on the way is refused. `clone` has no workspace boundary by design (a user-chosen destination).
