...
- `packages/web/server/lib/opencode/managed-catalog-guard.js`: managed project catalog boundary. When `OPENCHAMBER_MANAGED_CATALOG=1` (set by a managed launcher such as smarty-code), `POST /api/fs/clone` and `POST /api/opencode/directory` return 403 before any filesystem or settings change (`POST /api/fs/mkdir` is narrowed in its own route; see `../fs/DOCUMENTATION.md`), and `PUT /api/config/settings` returns 403 when `projects` contains a path that is not already saved. Bookmark edits and `lastDirectory`/`activeProjectId` writes still pass; `persistSettings` already repoints an unknown `activeProjectId`. The settings runtime also skips the legacy `lastDirectory` → project migration in this mode. Registered first by `feature-routes-runtime.js`. Unset means stock behavior.
