...
- `packages/web/server/lib/opencode/managed-catalog-guard.js`: managed project catalog boundary. When `OPENCHAMBER_MANAGED_CATALOG=1` (set by a managed launcher such as smarty-code), `POST /api/fs/clone` and `POST /api/opencode/directory` return 403 before any filesystem or settings change (`POST /api/fs/mkdir` is narrowed in its own route; see `../fs/DOCUMENTATION.md`), and `PUT /api/config/settings` returns 403 when `projects` contains a path that is not already saved. Bookmark edits still pass. A non-empty `lastDirectory` must be a live managed row, and a non-empty `activeProjectId` must name a saved bookmark whose path is a live row; otherwise 403 before persisting. If the live catalog cannot be read, these writes get 503. The settings runtime also skips the legacy `lastDirectory` → project migration in this mode. Registered first by `feature-routes-runtime.js`. Unset means stock behavior.
- `packages/web/server/lib/opencode/managed-catalog-reader.js`: `isLiveDirectory(directory)` reads the live rows on each call from the OpenCode gateway (`GET /project` with the OpenCode auth headers), requires `x-smarty-code-catalog: managed-v1` and valid rows, and compares canonical paths (real path when it exists, trailing slash tolerant). It throws when the catalog cannot be read. No cache or timer.
...
The root, `index.html`, `mobile.html`, `mini-chat.html`, SPA fallback and `sw.js`
return current complete files with `Cache-Control: no-store`. They ignore GET/HEAD
