...
describe('CLI diagnostics', () => {
  ...
  it('preserves upstream unhandled rejection serialization in JSON mode', async () => {
    const result = await runCliDiagnostic('rejection', true);

    expect(result.code).toBe(1);
    expect(result.stdout).toContain('"status": "error"');
    expect(result.stdout).toContain('"message": "Unhandled rejection: Error: diagnostic rejection"');
