...
Use this guide when running smarty-code behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm smarty-code works directly first.
...
- Disable gzip on the proxy if smarty-code is already compressing responses.
...
- smarty-code reachable directly on LAN
...
- smarty-code compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by smarty-code, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
