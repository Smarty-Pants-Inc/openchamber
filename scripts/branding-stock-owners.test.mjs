...
test('attribution overlay binds only its exact reviewed source without replacing donor evidence', () => {
  ...
  for (const file of attributionPaths) {
    ...
    assert.equal(entry.nativeCompletionSha256 ?? entry.nativeLifecycleSha256 ?? entry.nativeCreationSha256, entry.combinedSha256, file);
  }
...
test('native creation overlay binds its exact source and only the twelve attribution overlaps', () => {
  ...
  for (const file of attributionPaths) {
    ...
    assert.equal(entry.nativeCompletionSha256 ?? entry.nativeLifecycleSha256 ?? entry.nativeCreationSha256, sha256(read(file)), file);
  }
...
test('native draft lifecycle overlay preserves prior source evidence and extends only the existing store overlap', () => {
  ...
  assert.equal(entry.nativeCompletionSha256 ?? entry.nativeLifecycleSha256, sha256(read(entry.path)));
...
test('stock owners retain behavior except explicitly reviewed overlay and owned labels', () => {
  ...
  for (const { path: file, normalize, stockSha256 } of parity.files) {
    ...
    if (changed) {
      ...
      assert.equal(changed.nativeCompletionSha256 ?? changed.nativeLifecycleSha256 ?? changed.nativeCreationSha256 ?? changed.behaviorSha256, changed.combinedSha256, file);
